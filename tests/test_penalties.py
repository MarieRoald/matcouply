...
def test_row_vector_penalty_forwards_updates_correctly(rng, random_matrix, random_matrices):
    ...
    updated_factor_matrices = penalty.factor_matrices_update(random_matrices, np.ones(len(random_matrices)), random_matrices) # last input is ignored
...
class BaseTestADMMPenalty:
    ...
    def test_init_aux(self, rng, random_ragged_cmf, dual_init):
        ...
        penalty = self.PenaltyType(aux_init="random_standard_normal", dual_init=dual_init, **self.penalty_default_kwargs)
...
class TestParafac2(BaseTestFactorMatricesPenalty):
    ...
    def test_init_aux(self, rng, random_ragged_cmf, dual_init):
        ...
        penalty = self.PenaltyType(aux_init=(init_bases, init_coordinates), dual_init=dual_init, **self.penalty_default_kwargs)
