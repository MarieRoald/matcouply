...
def get_simple_simulated_data(noise_level=0.2, random_state=1):
    ...
    The entries in :math:`\mathbf{A}` are uniformly distributed between 0.1 and 1.1. This is done
    to ensure that there is signal from all components in all matrices.
    ...
    The entries in :math:`\mathbf{C}` (or :math:`\mathbf{D}_k`-matrices) are truncated normal
    distributed, and are therefore sparse.
